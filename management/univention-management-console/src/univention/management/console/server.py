...
from univention.management.console.oidc import OIDCLogin, OIDCLogout
...
class Server(object):
	...
	def run(self):
		...
		with open('/etc/machine.secret') as fd, open('/usr/share/univention-management-console/oidc/oidc.json') as fd2:
			config = json.load(fd2)
			settings = {
				'oidc': config.get('oidc'),
				'oidc_default_op': config.get('oidc_default_op'),
				"cookie_secret": fd.read().strip(),
				# "xsrf_cookies": True,  # enable?
				"debug": True,
			}
